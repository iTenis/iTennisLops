    ...
    public int ConfigPxeServer(JSchExecutor jSchUtil) throws Exception {
        ...
        new Thread(() -> {
            try {
                ...
                networkConfigService.SetNetwork(jSchUtil, fileUtils.getPxeConfigYmlWithpxeServer().get("interface"), fileUtils.getPxeConfigYmlWithpxeServer().get("ip"), fileUtils.getPxeConfigYmlWithpxeServer().get("netmask"));

                linkedQueue.add("配置yum源");
                yumConfigService.setMountISOWithConfig(jSchUtil, fileUtils.getPxeConfigYmlWithpxeServer().get("iso"), "/media");
                ...
                String osversionPath = fileUtils.getPxeConfigYmlWithinstallOS().get("os") + "/" + fileUtils.getPxeConfigYmlWithinstallOS().get("version");
                String serverip = fileUtils.getPxeConfigYmlWithdhcpServer().get("ip");
                String serveros = fileUtils.getPxeConfigYmlWithinstallOS().get("os");
                String serverversion = fileUtils.getPxeConfigYmlWithinstallOS().get("version");
                ...
                jSchUtil.execCmd("(\n" +
                        ...
                        "subnet " + fileUtils.getPxeConfigYmlWithdhcpServer().get("subnet") + " netmask " + fileUtils.getPxeConfigYmlWithdhcpServer().get("netmask") + " {\n" +
                        "        range " + fileUtils.getPxeConfigYmlWithdhcpServer().get("start") + " " + fileUtils.getPxeConfigYmlWithdhcpServer().get("end") + ";\n" +
                ...
                jSchUtil.execCmd("(\n" +
                        ...
                        "\tappend initrd=../images/" + osversionPath + "/initrd.img ip=dhcp repo=http://" + serverip + "/os/" + osversionPath + " ks=http://" + serverip + "/kickstarts/" + serveros + "_" + serverversion + "_ks.cfg ksdevice=" + fileUtils.getPxeConfigYmlWithinstallOS().get("interface") + "\n" +
                        "label manuallinux\n" +
                ...
                jSchUtil.execCmd("(\n" +
                        ...
                        "title " + serveros + " UEFI\n" +
                        "\troot (nd)\n" +
                        "        kernel /../images/" + osversionPath + "/vmlinuz ip=dhcp ks=http://" + serverip + "/kickstarts/" + serveros + "_" + serverversion + "_ks.cfg repo=http://" + serverip + "/os/" + osversionPath + " ksdevice=" + fileUtils.getPxeConfigYmlWithinstallOS().get("interface") + "\n" +
                ...
                jSchUtil.execCmd("(\n" +
                        ...
                        "        linuxefi /uefi/../images/" + osversionPath + "/vmlinuz ip=dhcp inst.repo=http://" + serverip + "/os/" + osversionPath + " inst.ks=http://" + serverip + "/kickstarts/" + serveros + "_" + serverversion + "_ks.cfg ksdevice=" + fileUtils.getPxeConfigYmlWithinstallOS().get("interface") + "\n" +
                        "        initrdefi /uefi/../images/" + osversionPath + "/initrd.img\n" +
                        "}\n" +
                        "menuentry 'Manual Install " + serveros + " Min UEFI' {\n" +
                        "        linuxefi /uefi/../images/" + osversionPath + "/vmlinuz ip=dhcp inst.repo=http://" + serverip + "/os/" + osversionPath + " ksdevice=" + fileUtils.getPxeConfigYmlWithinstallOS().get("interface") + "\n" +
                        "        initrdefi /uefi/../images/" + osversionPath + "/initrd.img\n" +
                        "}\n" +
                        "EOF\n" +
                ...
                jSchUtil.execCmd("(\n" +
                        ...
                        "rootpw --plaintext " + fileUtils.getPxeConfigYmlWithinstallOS().get("password") + "\n" +
                ...
                List<Map<String, String>> parts = fileUtils.getPxeConfigYmlWithpartitions();
                ...
                if (!"".equals(fileUtils.getPxeConfigYmlWithinstallOS().get("driver"))) {
                    linkedQueue.add("配置pxe过程中的引导驱动");
                    jSchUtil.execCmd("cp -rvf " + fileUtils.getPxeConfigYmlWithinstallOS().get("driver") + " /var/www/html/driver/");
                    jSchUtil.execCmd("sed -i \"10a\\driverdisk --source=http://" + serverip + "/driver/" + new File(fileUtils.getPxeConfigYmlWithinstallOS().get("driver")).getName() + "\" /var/www/html/kickstarts/" + serveros + "_" + serverversion + "_ks.cfg");
                }

                linkedQueue.add("配置子系统镜像");
                yumConfigService.setMountISOWithConfig(jSchUtil, fileUtils.getPxeConfigYmlWithinstallOS().get("iso"), "/var/www/html/os/" + osversionPath);
                ...
                if (fileUtils.getPxeConfigYmlWithpxeServer().get("version").startsWith("6")) {
